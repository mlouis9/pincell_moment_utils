...
def generate_unique_weight_combinations(num_values: int, num_weights: int=4)->Tuple[List[Tuple[float]], int]:
    """Generate unique (up to cyclic permutations, reflecting assumed rotational symmetry of boundary) weight combinations
    where w_1 + w_2 + w_3 + ⋯ + w_num_weights = 1 and each w_i comes from {0, 1/(num_values-1), ..., 1}

    Parameters
    ----------
    num_values
        The number of discrete values each weight is allowed to take on
    num_weights
        The number of weights (in each set) to generate. For 2D rectangular domains, this is 4.

    Returns
    -------
    weight_combinations
        A list of unique weight combinations
    num_combinations
        The number of unique weight combinations

    Notes
    -----
    The weights are generated by generating all integer partitions of n-1 into num_weights parts, where n = num_values.
    Each partition is then converted to a weight combination by dividing each part by n-1.
    The weight combinations are then sorted and only the lexicographically smallest cyclic permutation is kept.
    This ensures that each weight combination is unique up to cyclic permutations.
    The weight combinations are then sorted and returned as a list of tuples.
    """
    # Generate all integer partitions of n-1 into 4 parts
    all_partitions = [p for p in combinations_with_replacement(range(num_values), num_weights) if sum(p) == (num_values-1)]
    
    unique_representatives = set()
    
    for partition in all_partitions:
        # Generate all cyclic permutations
        cyclic_permutations = {tuple(partition[i:] + partition[:i]) for i in range(num_weights)}
        # Store only the lexicographically smallest one
        unique_representatives.add(min(cyclic_permutations))
